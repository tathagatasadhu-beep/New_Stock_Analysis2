...
def find_support_resistance_levels(df: pd.DataFrame, window: int = 10, min_touches: int = 2) -> Tuple[List[float], List[float]]:
...
def make_candlestick_with_sr(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        title="Candlestick with Support/Resistance",
